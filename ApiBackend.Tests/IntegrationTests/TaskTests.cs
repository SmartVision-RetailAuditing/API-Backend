...
{
    public TaskTests(TestFactory factory)
        : base(factory)
    {
    }
    ...
    {
        var fieldWorkerUser = GetTestFieldWorkerUser();
    ...
    public async Task GetTasksById_ShouldReturnOk_WithValidTaskId()
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new Exception($"GetTasksById failed ${errorText}");
        }
    }

    [Fact]
    public async Task GetTasksById_ShouldReturnNotFound_WithTaskIdOutOfBounds()
    {
        var fieldWorkerUser = GetTestFieldWorkerUser();
        LoginResponseDto user = await AuthenticateUser(fieldWorkerUser);
